@testset "Operations" begin
  ...
  # ============================================================
  # FusionRings.fusion_outcomes 
  # ============================================================

  @testset "FusionRings.fusion_outcomes" begin
    maybe_testset("basic", "1. basic construction") do
      z2 = zn_fusion_ring(2)
    ...
    maybe_testset("reference", "3. reference / Anyonica parity") do
      # No direct Anyonica fixture was exported for FusionRings.fusion_outcomes.
      ...
      for idx in oracle_case_indices("permuted_tabs.json", data)
        ...
        for j in eachindex(perms)
          ...
          check_mt_equal(
            actual,
            expected_tabs[j],
            "permute_mult_tab did not match Anyonica permuted_tabs.json for case $idx, with ring with code: $code, and permutation nr $j: $p",
          )
        end
      end
    end
  ...
  @testset "perm_vec_qd / perm_vec_sd_conj" begin
    ...
    maybe_testset("intermediate", "2. intermediate correctness") do
      ...
      check_equal(
        FusionRings.perm_vec_qd(z3; order = :decreasing),
        [1, 2, 3],
        "FusionRings.perm_vec_qd(z3; order=:decreasing) was not the identity permutation",
      )

      check_equal(
        FusionRings.perm_vec_sd_conj(z3),
        [1, 2, 3],
      ...
      return check_equal(
        ...
        [1, 3, 2, 4],
        "FusionRings.perm_vec_sd_conj(z4) did not place the self-dual object before the conjugate pair",
      )
    end
    ...
    maybe_testset("reference", "3. reference / Anyonica parity") do
      ...
      for idx in oracle_case_indices("tensor_product_tables.json", data)
        ...
        check_mt_equal(
          actual,
          data["Output"][idx],
          "tensor_product did not match Anyonica tensor_product_tables.json for case $idx, with rings with codes $code1, $code2",
        )
      end
    end
  end
