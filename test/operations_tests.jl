@testset "Operations" begin
  ...
  @testset "which_permutation / is_equivalent" begin
    maybe_testset("basic", "1. basic construction") do
      ...
      check_true(
        wp === missing || wp isa Vector{Int},
        "which_permutation(z3, pz3) returned neither missing nor a permutation vector",
      )
      return check_true(eq isa Bool, "is_equivalent(z3, pz3) did not return a Bool")
    end
    ...
    maybe_testset("intermediate", "2. intermediate correctness") do
      ...
      wp = which_permutation(z3, pz3)
      check_true(
        wp !== missing, "which_permutation(z3, permute(z3,[1,3,2])) returned missing"
      )
      check_equal(
      ...
      check_true(
        is_equivalent(z3, pz3), "is_equivalent(z3, permute(z3,[1,3,2])) returned false"
      )
      ...
      return check_false(
        is_equivalent(z2, z4), "is_equivalent(z2, z4) should have returned false"
      )
    end
