@testset "Formatting and printing" begin
    ...
    @testset "integer formatting helpers" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(b isa AbstractString,
                "bold_integer(12) did not return a string")
            check_true(s isa AbstractString,
                "subscript_integer(12) did not return a string")
            check_true(t isa AbstractString,
                "superscript_integer(12) did not return a string")

            check_false(isempty(b),
                "bold_integer(12) returned an empty string")
            check_false(isempty(s),
                "subscript_integer(12) returned an empty string")
            check_false(isempty(t),
                "superscript_integer(12) returned an empty string")
        end
    ...
    @testset "element_to_string" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(a isa AbstractString,
                "element_to_string(0, \"x\") did not return a string")
            check_true(b isa AbstractString,
                "element_to_string(1, \"x\") did not return a string")
            check_true(c isa AbstractString,
                "element_to_string(2, \"x\") did not return a string")
        end
    ...
    @testset "row_to_string" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(s isa AbstractString,
                "row_to_string did not return a string")
        end

        maybe_testset("intermediate", "2. intermediate correctness") do
            z3 = zn_fusion_ring(3)

            check_equal(row_to_string(z3, [1, 0, 0]), "0",
                "row_to_string(Z3, [1,0,0]) did not return \"0\"")
            check_equal(row_to_string(z3, [0, 1, 0]), "1",
                "row_to_string(Z3, [0,1,0]) did not return \"1\"")
            check_equal(row_to_string(z3, [0, 0, 1]), "2",
                "row_to_string(Z3, [0,0,1]) did not return \"2\"")
            check_equal(row_to_string(z3, [1, 0, 1]), "0 ⊕ 2",
                "row_to_string(Z3, [1,0,1]) did not join summands with ⊕")
            check_equal(row_to_string(z3, [2, 0, 1]), "2 0 ⊕ 2",
    ...
    @testset "product_string" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(s isa AbstractString,
                "product_string(Z2, 1, 1) did not return a string")
            check_false(isempty(s),
                "product_string(Z2, 1, 1) returned an empty string")
        end
    ...
    @testset "print_multiplication_table / pmt" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(tab isa AbstractMatrix,
                "print_multiplication_table(Z2) did not return a matrix-like object")
            check_equal(size(tab), (rank(z2), rank(z2)),
                "print_multiplication_table(Z2) did not return a rank×rank table")
            check_equal(size(tab_alias), (rank(z2), rank(z2)),
                "pmt(Z2) did not return a rank×rank table")
        end
        ...
        maybe_testset("intermediate", "2. intermediate correctness") do
            ...
            check_equal(tab2[1,1], "0",
                "print_multiplication_table(Z2)[1,1] was not \"0\"")
            check_equal(tab2[2,2], "0",
                "print_multiplication_table(Z2)[2,2] was not \"0\"")
            check_equal(tab2[1,2], "1",
                "print_multiplication_table(Z2)[1,2] was not \"1\"")

            tab3 = print_multiplication_table(z3)
            check_equal(tab3[2,2], "2",
                "print_multiplication_table(Z3)[2,2] was not \"2\"")
            check_equal(tab3[2,3], "0",
                "print_multiplication_table(Z3)[2,3] was not \"0\"")

            check_equal(pmt(z2), tab2,
                "pmt(z2) did not agree with print_multiplication_table(z2)")
        end
    ...
    @testset "Base.show(::FusionRing)" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(shown isa AbstractString,
                "show(zn_fusion_ring(2)) did not produce a string")
            check_false(isempty(shown),
                "show(zn_fusion_ring(2)) produced an empty string")
        end
        ...
        maybe_testset("intermediate", "2. intermediate correctness") do
            ...
            check_true(startswith(shown_named, "FR("),
                "show(zn_fusion_ring(2)) did not begin with \"FR(\"")
            check_true(occursin("Z_2", shown_named),
                "show(zn_fusion_ring(2)) did not contain the ring name")
            check_true(endswith(shown_named, ")"),
            ...
            check_true(startswith(shown_unnamed, "FR("),
                "show on an unnamed fusion ring did not begin with \"FR(\"")
            check_true(endswith(shown_unnamed, ")"),
    ...
    @testset "string cleanup helpers" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(f isa AbstractString, "fix_poly_string did not return a string")
        end
    ...
    @testset "small algebraic formatting helpers" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(fs isa Tuple,
                "factor_squares(12) did not return a tuple")
            check_true(g1 isa Bool,
                "is_geometric_array([1,2,4,8]) did not return a Bool")
        end
        ...
        maybe_testset("intermediate", "2. intermediate correctness") do
            ...
            check_true(is_power_sum(x^2 + x + 1),
                "is_power_sum(x^2 + x + 1) should be true")
            check_false(is_power_sum(x^2 + 2*x + 1),
    ...
    @testset "default labels use bold_integer" begin
        maybe_testset("basic", "1. basic construction") do
            ...
            check_true(r isa FusionRing,
                "fusion_ring(mt) did not return a FusionRing")
